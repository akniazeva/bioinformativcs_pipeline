...
AreaucDataTest <- array()
PerfonamceDataTest <- array()

lskVariable100100 <- replicate(2, array(sample(intersect(metabolic.signatures$lskVariable,metabolic.signatures$allGenesTested), 100, replace = F )))

for (i in names(lskVariable100100)) {
...
#see how well we can predict the fate of precurosrs
  ...
  AreaucDataTest <- append(numericAuc, i)
  PerfonamceDataTest <- append(performance, i)
