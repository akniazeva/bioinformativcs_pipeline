...
lskVariable100100 <- replicate(100, array(sample(metabolic.signatures$lskVariable,metabolic.signatures$allGenesTested), 100, replace = F )))

testinggenes <- replicate(10, array(sample(rownames(haem.seurat)), 100, replace = F ))



for (i in 1:ncol(lskVariable100100)) {

  interesting.genes <- lskVariable100100[,i]
...
#prep the inputs for xgboost, im tempted to retry with the progenitors included in the training just to see how that translates to the single cell
  ...
  AreaucDataTest100 <- append(AreaucDataTest100, areauc)
  
  }
  
  AreaucDataTest100#resuts from 100 sets by 100 genes
