...
p1 <- DimPlot(object = haem.seurat, reduction = "pca", group.by = "lineage", pt.size = 1.5, label.size = 4,  label = T, repel = T) 
p2 <- DimPlot(object = haem.seurat, reduction = "umap", group.by = "lineage", pt.size = 1.7, label.size = 4,  label = T, repel = T)
...
reg_test_immuno <- intersect(reg.immuno, metabolic.signatures$allGenesTested)

#get all immuno genes

immuno.genes <- (all.immuno.genes[,2])

immuno <- intersect(rownames(haem.seurat), immuno.genes) #the same number of genes in after intersect with seurat dataset as with immuno regulatory genes
...
limma.volcano.immuno <- function(res, main="", fct=2, pt=0.05){
  ...
  res$sign[which(res$adj.P.Val < pt & abs(res$logFC) > fct & res$genes %in% reg_test_immuno)] <- 3
